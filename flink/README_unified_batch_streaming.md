...
|:---------------------------------------|:--------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| tidb.source.failover                   | split         | The TiDB batch stage recovery strategy when it fails, `split` and `offset` are available. `split` recovers from the failed region, guaranteeing exactly once semantics, and `offset` recovers from the failed single row data, guaranteeing exactly once semantics. Note that the `offset` behavior may change in higher versions of TiDB (6+), making it impossible to use. |
