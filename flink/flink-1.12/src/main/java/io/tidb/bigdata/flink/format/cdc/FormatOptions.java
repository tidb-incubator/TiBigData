...
public class FormatOptions {
  ...
  private static Optional<Stream<String>> getOptionalStream(
      final ReadableConfig config, final ConfigOption<String> key) {
    return config
        .getOptional(key)
        .map(l -> Arrays.stream(l.split("[ ]*[,;|][ ]*")).filter(FormatOptions::isNotEmpty));
  }

  static Set<String> getOptionalSet(final ReadableConfig config, final ConfigOption<String> key) {
    return getOptionalStream(config, key).map(l -> l.collect(Collectors.toSet())).orElse(null);
  }

  static <R> Set<R> getOptionalSet(
    ...
    return getOptionalStream(config, key)
        .map(s -> s.map(mapper).collect(Collectors.toSet()))
        .orElse(null);
  }

  static long getEarliestTs(final ReadableConfig config) {
    return config
        .getOptional(EARLIEST_VERSION)
