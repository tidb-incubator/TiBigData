...
public class TiDBSinkFunction extends
  ...
  private transient Map<String, TiDBWriteHelper> tiDBWriteHelpers;
  private transient Map<String, TiDBEncodeHelper> tiDBEncodeHelpers;
  ...
  private void makeSureInit() {
    ...
    Long start = rowIdStarts.get(getRuntimeContext().getIndexOfThisSubtask());
  ...
  protected TiDBTransactionState beginTransaction() throws Exception {
    ...
    TiDBEncodeHelper tiDBEncodeHelper = new TiDBEncodeHelper(session, rowIDAllocator, timestamp,
        databaseName, tableName, sinkOptions.isIgnoreAutoincrementColumn());
