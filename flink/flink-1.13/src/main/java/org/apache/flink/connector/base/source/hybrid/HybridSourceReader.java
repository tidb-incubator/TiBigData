...
public class HybridSourceReader<T> implements SourceReader<T, HybridSourceSplit> {
  ...
  public void start() {
    ...
    readerContext.sendSourceEventToCoordinator(new SourceReaderFinishedEvent(initialSourceIndex));
  }
  ...
  public List<HybridSourceSplit> snapshotState(long checkpointId) {
    List<? extends SourceSplit> state =
        currentReader != null ? currentReader.snapshotState(checkpointId) : Collections.emptyList();
