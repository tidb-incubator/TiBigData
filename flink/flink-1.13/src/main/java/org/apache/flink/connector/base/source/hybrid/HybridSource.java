...
public class HybridSource<T> implements Source<T, HybridSourceSplit, HybridSourceEnumeratorState> {
  ...
  public SplitEnumerator<HybridSourceSplit, HybridSourceEnumeratorState> restoreEnumerator(
      SplitEnumeratorContext<HybridSourceSplit> enumContext, HybridSourceEnumeratorState checkpoint)
      throws Exception {
   ...
   *
   * <p>To derive a start position at switch time, the source can be initialized from context of the
   * previous enumerator. A specific enumerator implementation may carry state such as an end
   * timestamp, that can be used to derive the start position of the next source.
   *
   * <p>Currently only the previous enumerator is exposed. The context interface allows for backward
   * compatible extension, i.e. additional information about the previous source can be supplied in
   * the future.
   */
   ...
   * time. Provides the ability to set a start position in any way a specific source allows. Future
   * convenience could be built on top of it, for example a default implementation that recognizes
   * optional interfaces to transfer position in a universal format.
   *
   * <p>Called when the current enumerator has finished. The previous source's final state can thus
   * be used to construct the next source, as required for dynamic position transfer at time of
   * switching.
   *
  ...
  public static class HybridSourceBuilder<T, EnumT extends SplitEnumerator>
    ...
    public <ToEnumT extends SplitEnumerator, NextSourceT extends Source<T, ?, ?>>
        HybridSourceBuilder<T, ToEnumT> addSource(
            SourceFactory<T, NextSourceT, ? super EnumT> sourceFactory, Boundedness boundedness) {
      if (!sources.isEmpty()) {
