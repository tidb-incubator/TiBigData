...
public class HybridSourceSplitEnumerator
        ...
        (k, splitsPerSource) -> {
          if (k == currentSourceIndex) {
            currentEnumerator.addSplitsBack(
                HybridSourceSplit.unwrapSplits(splitsPerSource, switchedSources), subtaskId);
          } else {
            pendingSplits
  ...
  private void sendSwitchSourceEvent(int subtaskId, int sourceIndex) {
    ...
    context.sendEventToSourceReader(
        subtaskId, new SwitchSourceEvent(sourceIndex, source, sourceIndex >= (sources.size() - 1)));
  ...
  public HybridSourceEnumeratorState snapshotState(long checkpointId) throws Exception {
    ...
    return new HybridSourceEnumeratorState(
        currentSourceIndex, enumStateBytes, currentEnumeratorCheckpointSerializer.getVersion());
  }
  ...
  public void handleSourceEvent(int subtaskId, SourceEvent sourceEvent) {
    LOG.debug(
        "handleSourceEvent {} subtask={} pendingSplits={}", sourceEvent, subtaskId, pendingSplits);
      ...
      int subtaskSourceIndex =
          readerSourceIndex.computeIfAbsent(
              ...
              k -> {
                ...
                LOG.debug("New reader subtask={} sourceIndex={}", subtaskId, srfe.sourceIndex());
   ...
   *
   * <p>This context is used to wrap the splits into {@link HybridSourceSplit} and track assignment
   * to readers.
   */
